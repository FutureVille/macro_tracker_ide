...
import { useAppStore, calculateMacros, getTodayString } from "@/lib/store"
...
export default function HistoryPage() {
    const { weightHistory, logWeight, logs, foods, goals } = useAppStore()
    ...
    const handleLogWeight = () => {
        if (!newWeight) return
        logWeight(todayString, parseFloat(newWeight))
        setNewWeight("")
    ...
    }, [weightHistory, dateRange])

    // Calculate macro averages
    const macroAverages = useMemo(() => {
        const logsInRange = logs.filter((l) => l.date >= dateRange.start && l.date <= dateRange.end)
        ...
        logsInRange.forEach((log) => {
            ...
            const existing = dateMap.get(log.date) || { protein: 0, carbs: 0, fat: 0, calories: 0 }
    ...
    const todayWeight = weightHistory.find((w) => w.date === todayString)
        ...
        <div className="flex flex-col min-h-screen">
            ...
            <Modal isOpen={weightModalOpen} onClose={() => setWeightModalOpen(false)} title="Log Today's Weight">
                <div className="space-y-4">
                    ...
                    <Button onClick={handleLogWeight} className="w-full" disabled={!newWeight}>
                        Save Weight
                    </Button>
                </div>
            </Modal>
